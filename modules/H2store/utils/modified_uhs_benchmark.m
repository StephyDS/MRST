...
description = ['Modified benchmark for hydrogen storage with ', ...
    ...
    month = 365 * day / 12;   % Average number of days per month
    K0 = 273.15 * Kelvin;      % Absolute zero temperature in Kelvin
    ...
    [~, model, ~] = initEclipseProblemAD(deck,'getSchedule',false);
    fluid = model.fluid;
    deck = model.inputdata;
    rock.regions.saturation=deck.REGIONS.SATNUM;
    gravity reset on;
    % We use a blackoil model and "oil" is the water phase 
...
function W = setUpWells(G, rock, fluid, options)
    ...
    % Well coordinates (wc) and radii (r)
...
function schedule = setUpSchedule2(G0, rock, fluid, options)
    ...
    if options.use_cushion
        if options.use_bhp
            ...
            for i = 1:9
                dtCushion = dtCushions(i);
                W(1).val = rateCushion(10);  % Fixed rate for first steps
                scheduleCushions{i} = simpleSchedule(dtCushion, 'W', W);
            end
...
function bc = setUpBc(G, rock, fluid, options)
    % Initialize a single-phase fluid with specified viscosity (mu) and density (rho)
    fluid = initSingleFluid('mu', 0.89 * centi * poise, ...
                            'rho', 1005.207150 * kilogram / meter^3);

    % Set up saturation (sat) and compute fluid properties
    sat = 1;
    [mu, rho] = fluid.properties(struct('s', sat));
    
    % Compute relative permeability (kr) and mobility (mob)
    kr = fluid.relperm(sat);
    mob = bsxfun(@rdivide, kr, mu);  % Mobility: relative perm / viscosity

    % Compute omega, a parameter influenced by mobility and density
    omega = dot(mob, rho) ./ sum(mob, 2);

    % Set gravity direction based on grid dimension
    if G.griddim < 3
        grav_ = [0 -norm(gravity)];  % 2D case: gravity along y-axis
    else
        grav_ = gravity;  % 3D case: standard gravity vector
    end

    % Get boundary faces of the grid
    f = boundaryFaces(G);
    
    % Select faces where the z-component of the normal is less than a threshold (filtering out certain faces)
    f1 = (G.faces.normals(f, 3) > 2.48);
    faces = f(~f1);  % Exclude filtered faces
    ...
    dp = omega .* (dx * reshape(grav_, [], 1));
    
    % Define the pressure at the boundary (81.6 barsa plus the gravity-induced pressure drop)
    pressure = 81.6 * barsa() + dp;

    % Set the boundary condition with the computed pressure and saturation
    sat = 1;  % Saturation is full (single phase)
    bc = addBC([], faces, 'pressure', pressure, 'sat', sat);

    % Add zero saturation component for the gas phase (0)
    bc.sat = [bc.sat 0 * bc.sat];
