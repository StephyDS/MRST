classdef DispersionFlowDiscretization < FlowDiscretization
%Discretization and state function grouping for bio-chemistry flow
    ...
    methods
        %-----------------------------------------------------------------%
        function props = DispersionFlowDiscretization(model)
        ...
        function [acc, flux, names, types] = componentConservationEquations(fd, model, state, state0, dt)
            ...
            nph = sum(act); 
            if model.dispersion      
                J = model.getProps(flowState, 'DispersionPhaseFlux');
                for c = 1:ncomp
                    for ph = 1:nph                   
