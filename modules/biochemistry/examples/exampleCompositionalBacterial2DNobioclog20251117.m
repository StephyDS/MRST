...
bacteriamodel = true;
clogModel = false;%true;%
nbact0 = 5; % Initial bacteriastate0
if clogModel     
    nc_bact = 120;
...
arg = {model.G, model.rock, model.fluid, compFluid,...
    ...
    'bacteriamodel', bacteriamodel, 'moleculardiffusion',moldiff,'dispersion',disp,...
    'bactdiffusion',true,'chemotaxisEffect',chemotaxis,...
    'liquidPhase', 'O', 'vaporPhase', 'G'}; % Set phases and EOS model
model = BiochemistryModel(arg{:});
model.gravity = modelBo.gravity;

%% Initialize compositional state
%if bacteriamodel
    state0_bact = initCompositionalStateBacteria(model, state0.pressure, T0, state0.s, comp0, nbact0, EOS);
%else
    state0_nobact = initCompositionalState(model, state0.pressure, T0, state0.s, comp0, EOS);
%end
...
if clogModel % --- With bio-clogging
    problemWithClogging = packSimulationProblem(state0_bact, model, schedule, caseNameWithClogging, 'NonLinearSolver', nls);
    simulatePackedProblem(problemWithClogging);
else % --- Without clogging
    ...
    'bacteriamodel', true, 'moleculardiffusion',moldiff,'dispersion',disp,...
    'bactdiffusion',bactdiff,'chemotaxisEffect',chemotaxis,...
    'liquidPhase', 'O', 'vaporPhase', 'G');
    state0NoClogging = state0_bact;
    caseNameNoClogging = [baseName '_NO_CLOGGING_BACTDIFF'];
    problemNoClogging = packSimulationProblem(state0NoClogging, modelNoClogging, schedule, ...
        caseNameNoClogging, 'NonLinearSolver', nls);
...
false, DiagonalAutoDiffBackend('modifyOperators', true), 'oil', true, 'gas', true, ...
'bacteriamodel', false,'moleculardiffusion',moldiff,'dispersion',disp,...
'liquidPhase', 'O', 'vaporPhase', 'G');
state0NoBact = state0_nobact;
...
if bacteriamodel 
    if clogModel % --- With bio-clogging
        ...
        % Plot well outputs
        figure; plotWellSols({wsWithClog});
    else % --- Without clogging
