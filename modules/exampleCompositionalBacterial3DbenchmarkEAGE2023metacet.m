...
%% Fluid Properties
 ...
 %compFluid = TableCompositionalMixture({'Water', 'Hydrogen', 'CarbonDioxide', 'AceticAcid'}, ...
 %                                     {'H2O', 'H2', 'CO2','CH3COOH'});
 %biochemFluid = TableBioChemMixture({'AcetogenicBacteria'},{'nbactA'});
...
problem_nobact = packSimulationProblem(state0_nobact, model_nobact, schedule, ...
    'Benchmark_NoBacteria_15_metacet', 'NonLinearSolver', nls);
...
problem_bact = packSimulationProblem(state0_bact, model_bact, schedule, ...
    'Benchmark_Bacteriatace_15_metacet', 'NonLinearSolver', nls);
...
for i=1:nbioreact
    if strcmp(model_bact.biochemFluid.metabolicReaction(i), 'MethanogenicArchae')
        C1_gain  = (abs(results_nobact.totMassC1 - results_bact.totMassC1)...
            ./ results_nobact.totMassC1) * 100;
