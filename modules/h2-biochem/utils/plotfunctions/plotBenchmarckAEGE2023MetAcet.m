...
%       4) the microbial populations during time (methanogens and
%       acetogens)
%
%
% PARAMETERS:
...
%   totMassH2bact - total mass of product (CH3COOH or C1) with microbial activity
...
%Total mass of product CH3COOH or C1
f20=figure('Name','results_nobact.totMassCH3COOH','NumberTitle','off');
f20.Position(3:4) = [900 700];
plot(1:nT,totMassProdbact,'b','MarkerSize',7,'LineWidth',2)
title(' CH3COOH total mass over time','FontSize',16,'FontWeight','bold','Color','k')
xlabel({'time (days)'},'FontWeight','bold','Color','k')
ylabel({'CH3COOH mass (kg)'},'FontWeight','bold','Color','k')
ax = gca;
ax.FontSize = 16;
legend({'CH3COOH total mass, archae'},...
    'FontSize',16,'TextColor','black','Location','west')
...
for j=1:nbioreact
    for i = 1:nT
        nbacteria(i,j)=sum(statesBact{i}.nbact(:,j));
    end
end
